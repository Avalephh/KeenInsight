...
#     latL = []
#     for line in lines[1:-1]:  # 忽略最后一行
#         if line.strip() == '':
#             continue
#         tmp = line.split('\t')[-1].strip()
#         latL.append(float(tmp) / 1000)  # 转换为秒
...
#     lat = np.percentile(latL, 95)
#     total_time = min(timeout, np.max(latL))
#     tps = len(latL) * 60.0 / total_time

#     lat_var = statistics.variance(latL)
    
#     # 总执行时间
#     last_line = lines[-1].strip()
#     if last_line.startswith("Total Execution Time"):
#         total_time_ms = float(last_line.split('\t')[-1].split()[0])
    
#     total_time_s = total_time_ms / 1000

#     return [tps, total_time_s, tps, -1, lat_var, -1]

def get_total_execution_time(file_path,timeout=4):
    """ 读取文件并返回总执行时间（秒） """
    with open(file_path) as f:
        lines = f.readlines()
    if not lines:
    ...
    if last_line.startswith("Total Execution Time"):
        total_time_ms = float(last_line.split('\t')[-1].split()[0])
        return total_time_ms / 1000  # 转换为秒
    return 0
