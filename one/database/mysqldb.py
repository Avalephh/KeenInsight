...
from database.knobs import logger, initialize_knobs, get_default_knobs
from database.parser import ConfigParser
...
class MysqlDB:
    ...
    def _gen_config_file(self, knobs):
        ...
        for key in knobs.keys():
            ...
            # print("key: ", key, "value: ", knobs[key])

        cnf_parser.replace('/etc/my.cnf.d/my.cnf')
        # cnf_parser.replace('/etc/mysql/mysql.conf.d/mysqld.cnf')
    ...
    def _kill_mysqld(self):
        ...
        kill_cmd = '{} -u{} -p{} -S {} shutdown'.format(mysqladmin, self.user,self.passwd,self.sock)
    ...
    def apply_knobs_offline(self, knobs):
        print("apply_knobs_offline",knobs)
    ...
    def drop_and_create_db(self):
        db_conn = MysqlConnector(**self.connection_info)
        db_conn.execute('DROP DATABASE IF EXISTS {};'.format(self.dbname))
        print('DROP DATABASE IF EXISTS {};'.format(self.dbname))
        db_conn.execute('CREATE DATABASE {};'.format(self.dbname))
        print('CREATE DATABASE {};'.format(self.dbname))
        db_conn.close_db()
        logger.info('drop and create db done')
        return True
    ...
    def get_data(self):
        ...
        res = db_conn.fetch_results(sql, json=False)
        # 将结果转换为字典形式
        config_params = {row[0]: row[1] for row in res}
        db_conn.close_db()
        return config_params
    
    def get_config_info(self, knobs):
        db_conn = MysqlConnector(**self.connection_info)
        res = {}
        for key in knobs.keys():
            sql = "SHOW VARIABLES LIKE '{}';".format(key)
            r = db_conn.fetch_results(sql)
            # print(r)
            if r:
                res[key] = r[0]['Value']
    ...
    def clean_binlog(self):
        db_conn = MysqlConnector(**self.connection_info)
        sql = 'PURGE BINARY LOGS BEFORE "2025-11-15 00:00:00";'
        db_conn.execute(sql)
        db_conn.close_db()
        logger.info('clean binlog done')
        return True
