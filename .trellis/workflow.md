...
### Step 4: Prefer Task Worktrees

This repository uses a **worktree-first** implementation workflow:

```bash
python3 ./.trellis/scripts/session_worktree.py open <task-dir>
```

This command creates or reuses a dedicated worktree for the task, initializes a
`task/<slug>` branch when needed, and sets `.current-task` in both the source
repo and the worktree so subsequent implementation stays isolated.
...
|   |-- session_worktree.py  # Open/land per-task worktrees for this session
...
# Open or reuse the task worktree, then keep coding inside that path
python3 ./.trellis/scripts/session_worktree.py open <task-dir>
```
...
6. Land task branch and cleanup worktree
   --> python3 ./.trellis/scripts/session_worktree.py land <task-dir>
```
...
### Land Worktree After Recording

After recording the session, land the task branch back to its `base_branch`
(usually the branch you were on when opening the worktree) and remove the task
worktree:
...
This command:
1. Requires both the source repo and the task worktree to be clean
2. Fast-forwards the task branch into `base_branch`
3. Removes the task worktree
4. Deletes the task branch by default
5. Archives the task metadata under `.trellis/tasks/archive/`
...
python3 ./.trellis/scripts/session_worktree.py open <task-dir>         # Open/reuse task worktree
python3 ./.trellis/scripts/session_worktree.py land <task-dir>         # Merge task branch + cleanup
...
python3 ./.trellis/scripts/session_worktree.py open <task-dir>  # Open task worktree
python3 ./.trellis/scripts/session_worktree.py land <task-dir>  # Merge + cleanup
