...
本章将在需求分析的基础上，对数据库流量回放系统的整体架构、功能模块、性能指标及底层数据存储进行详细设计。系统旨在构建一个高效、准确且可视化的流量回放平台，核心目标是实现对多种数据库审计日志（如 PostgreSQL pgaudit、MySQL 审计插件等）的深度解析，并在目标数据库环境中高保真地重放业务流量，同时通过自动化的差异比对机制验证执行结果的一致性。

\subsection{功能需求分析}

通过对数据库测试与验证场景的深入剖析，本系统将核心功能划分为日志处理、流量控制、差异检测与报告生成四大逻辑单元，各单元协同工作以实现完整的闭环测试。

首先，\textbf{日志解析与预处理 (Log Parsing)} 模块是系统的入口。系统通过插件化解析器架构支持多种日志格式，当前已实现对 PgAudit 格式的解析器作为参考实现，能够上传并解析大规模日志文件，从中精准提取时间戳、Session ID、事务 ID、SQL 语句及绑定参数等关键元数据。针对参数化查询，解析引擎会自动识别 \$1、\$2 等占位符并填充实际参数值，将非结构化的日志重组为可执行的标准 SQL 语句。同时，为了保证业务逻辑的完整性，系统会依据事务 ID（Transaction ID）对 SQL 进行分组，确保原子性操作在回放时不会被打散。
...
最后，\textbf{报告生成 (Reporting)} 模块负责任务的总结输出。当回放任务结束后，系统将自动化生成两类分析报告：其一是在同构环境下用于验证回放器本身的“回放保真报告”，覆盖执行状态一致率、事务结果一致率、窗口吞吐误差、到达间隔分布差异、活跃事务曲线偏差以及 SQL 模板组成偏差等指标；其二是在异构环境下用于支撑数据库升级、参数调优和硬件迁移决策的“变更评估报告”，按 SQL 模板或事务类型输出 P95/P99 时延变化、吞吐变化、错误率变化与资源利用率变化，并附带详细的差异定位信息。
...
在\textbf{评测可重复性与观测扰动}方面，系统不仅要“回放得快”，还要“测得准”。因此，评估链路必须能够基于统一时间窗稳定采样 TPS/QPS、活跃事务数和时延分位数，且采样与聚合本身的额外开销应尽量低，以免统计逻辑反过来影响回放过程。同一任务在相同输入、相同初始快照和相同配置下重复执行时，其关键指标波动应控制在可解释范围内，从而保证评测结果具备可重复、可比较和可审计的工程价值。

\subsection{问题建模与核心抽象}

为了避免后续章节在介绍日志解析、事务重组与回放调度时反复回到概念层讨论，本文首先对系统处理对象作统一抽象。对于任意来源的数据库审计日志，只要解析器能够将其转换为统一的“回放单元”（Replay Unit）序列，后续的流量处理与并发回放过程便可在同一形式化框架下展开。
...
\end{equation}
其中，每个日志条目 $l_i$ 可视为一个“回放单元”，包含了执行该操作所需的核心信息：
\begin{equation}
...
因此，数据库流量回放系统在建模层面的核心任务，就是实现一个函数 $\mathcal{F}$，将输入的 $S_{log}$ 转化为一个由若干可回放事务组成的集合 $\mathcal{T}_{replay}$：
\begin{equation}
    \mathcal{F}: S_{log} \rightarrow \mathcal{T}_{replay} = \{ T_1, T_2, \dots, T_M \}
\end{equation}

上述形式化定义明确了本文后续各章节的分工：第 4 章重点讨论如何从异构日志中构造并重组这些回放单元，第 5 章则基于该抽象验证系统实现与实验评估结果。

\subsection{系统架构}

系统采用经典的分层架构设计（Layered Architecture）\cite{CleanArchitecture}，自上而下划分为接入层、服务层、核心逻辑层与数据持久层。层级之间遵循低耦合原则，通过清晰的接口定义进行交互。

\textbf{接入层 (Web/API Layer)} 基于 Gin 框架\cite{GinFramework}构建 RESTful API，作为系统的对外门户。它主要负责接收用户的任务指令，进行参数校验，并统一返回 JSON 格式的响应结果。其中，\texttt{internal/handler} 包定义了 \texttt{ReplayHandler}，用于处理具体的业务路由。
...
\textbf{数据层 (Data/Repository Layer)} 基于 GORM 框架\cite{GORM}实现元数据的持久化。\texttt{internal/repository} 封装了对持久化存储的 CRUD 操作，而 \textbf{Target DB Adapter} 则负责与被测目标数据库建立物理连接并执行 SQL。
\textbf{执行控制}：Target DB Adapter 内部维护一个带连接池（Connection Pool）\cite{ConnectionPooling}的并发执行队列。它从数据层拉取回放队列中的任务单元，依据调度算法指派工作协程并发执行，并实时收集 SQL 执行的结果（受影响行数、响应时间、错误码）。
...
本系统选用 PostgreSQL 作为系统自身元数据的存储引擎\cite{PostgreSQLDoc}。PostgreSQL 不仅支持标准的 SQL 规范，具备优秀的 ACID 事务特性，还提供了强大的 JSONB 数据类型支持，能够高效存储和查询回放任务的元数据。

数据库设计遵循第三范式（3NF）原则\cite{DatabaseSystemConcepts}，以减少数据冗余并保证数据的一致性。同时，考虑到流量回放场景下存在海量日志数据的写入与聚合查询需求，针对核心查询字段（如 \texttt{task\_id}、\texttt{sql\_id}、\texttt{round}）建立了必要的 B-Tree 索引，以提升检索性能。所有主键 ID 均采用 UUID 标准（36位字符），以确保在分布式环境下的唯一性。
...
本章从系统层面对数据库流量回放平台进行了总体设计。首先围绕日志解析、流量控制、差异校验与两类报告生成梳理了功能需求，并明确了高并发、低延迟、流式处理、可重复观测等性能目标；随后给出了面向后续章节的统一问题建模与核心抽象，明确了“回放单元—事务—会话”的基本处理对象；在此基础上，进一步完成了系统分层架构与元数据存储结构设计。上述内容共同构成了全文的设计基线，为第 4 章展开关键技术实现、第 5 章开展平台落地、保真验证与变更评估提供了统一前提。
