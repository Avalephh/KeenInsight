...
\usepackage{listings}
\usepackage{graphicx}
% \usepackage{epstopdf} % 移除此行，因为项目中所有图片均为 pdf 格式，且 XeLaTeX 不支持其驱动
\usepackage{booktabs}
\usepackage{multirow}
\usepackage{geometry}
\usepackage{appendix}
...
%添加封面
\include{textfiles/Cover}

%%添加摘要
\include{textfiles/0abstract}
...
\input{textfiles/1introduction}
\input{textfiles/2Related_work}
\input{textfiles/3FirstResearch}
\input{textfiles/4SecondResearch}
\input{textfiles/5ThirdResearch}
\input{textfiles/6Conclusion}
\input{textfiles/Summary}
...
\input{textfiles/7Acknowledgement}
