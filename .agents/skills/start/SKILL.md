...
### Step 5: Default Session Isolation via Worktree

For development tasks in this repository, default to a dedicated task
worktree after the task directory and PRD are ready:

```bash
python3 ./.trellis/scripts/session_worktree.py open "$TASK_DIR"
```

Use the returned path as the working directory for subsequent implementation
...
This opens a dedicated worktree, syncs the task directory into it, and sets
`.current-task` in both the source repo and the worktree.
...
| `python3 ./.trellis/scripts/session_worktree.py open` | Open/reuse a task worktree and set current task |
